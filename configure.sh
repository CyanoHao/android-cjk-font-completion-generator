...
out/$file: out/$basename.otd cjk/$cjkbase.otd
	python merge.py out/$basename.otd cjk/$cjkbase.otd
...
for file in NotoSansCJKsc-{Thin,Light,Regular,Medium,Bold,Black}.otd NotoSansCJKscCondensed-{Light,Regular,Medium,Bold}.otd; do
	...
	cat >>Makefile <<EOF
cjk/$italic: cjk/$file
	python italize.py cjk/$file cjk/$italic 0.1733
EOF
done
for file in NotoSerifCJKsc-{Regular,Bold}.otd; do
	...
	cat >>Makefile <<EOF
cjk/$italic: cjk/$file
	python italize.py cjk/$file cjk/$italic 0.2125
EOF
done
...
cjk/$condensed: cjk/$file
	python condense.py cjk/$file cjk/$condensed
EOF
done
